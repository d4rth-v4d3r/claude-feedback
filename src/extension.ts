...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("codeReview.sendInCurrentTerminal", async () => {
...
class CodeReviewSidebarProvider implements vscode.WebviewViewProvider {
  ...
  async sendForReview(workspaceFolderPath?: string): Promise<void> {
    ...
    if (!cwd) {
      vscode.window.showWarningMessage(
        "Pending comments span multiple workspace folders. Use the send actions under each worktree in the sidebar."
      );
      return;
    }
  ...
  async sendInCurrentTerminal(workspaceFolderPath?: string): Promise<void> {
  ...
  private getTargetCommentsForSend(workspaceFolderPath?: string): ReviewComment[] {
