...
import { commentBodyForSend } from "./store";
...
function showInlinePending(): boolean {
  return vscode.workspace.getConfiguration("codeReview").get<boolean>("showInlinePending", true);
}
 ...
 */
export class ReviewEditorHints implements vscode.Disposable {
  ...
  constructor(
    ...
    private readonly getPending: () => ReviewComment[],
  ) {
    ...
    this.disposables.push(
      ...
      vscode.window.onDidChangeTextEditorSelection(() => this.raiseCodeLens()),
      vscode.window.onDidChangeActiveTextEditor(() => this.raiseCodeLens()),
  ...
  private applyDecorations(editor: vscode.TextEditor, pending: ReviewComment[]): void {
    const fsPath = editor.document.uri.fsPath;
    const forFile = pending.filter((c) => c.filePath === fsPath);
    const opts: vscode.DecorationOptions[] = forFile.map((c) => {
      ...
      const body = commentBodyForSend(c);
    ...
    for (const c of pending) {
      ...
      lenses.push(
        new vscode.CodeLens(range, {
          title: `$(comment-discussion) ${truncate(commentBodyForSend(c), LENS_CHARS)}`,
  ...
  private sortLenses(lenses: vscode.CodeLens[]): vscode.CodeLens[] {
