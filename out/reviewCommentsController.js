...
class ReviewCommentsController {
    constructor(store, addCommentForUri) {
        this.store = store;
        this.addCommentForUri = addCommentForUri;
        ...
        this.disposables.push(vscode.workspace.onDidSaveTextDocument((doc) => this.reanchorOnSave(doc)));
    ...
    async appendMessage(commentId, body) {
    ...
    async editMessage(commentId, messageId, nextBody) {
    ...
    async deleteMessage(commentId, messageId) {
        await this.store.update((state) => {
            const c = state.pending.find((p) => p.id === commentId);
            if (!c) {
                return;
    ...
    createThread(comment) {
        ...
        const range = new vscode.Range(Math.max(0, comment.line - 1), 0, Math.max(0, comment.line - 1), 0);
        const thread = this.controller.createCommentThread(uri, range, this.toVscodeComments(comment));
        this.applyToThread(thread, comment);
        return thread;
    }
    applyToThread(thread, comment) {
...
function isReviewComment(c) {
    return typeof c.reviewCommentId === "string";
}
/**
 ...
 */
function locateAnchorLine(documentLines, comment) {
...
function stripContextMarker(line) {
...
//# sourceMappingURL=reviewCommentsController.js.map