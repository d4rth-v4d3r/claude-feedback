...
class CodeReviewSidebarProvider {
    ...
    async sendForReview(workspaceFolderPath) {
        ...
        if (usedClaudeCommand) {
            ...
            if (!sawLaunch) {
                vscode.window.showWarningMessage("Claude did not report as started in time; pasted anyway. If zsh ran part of the review text, paste from the clipboard after Claude is ready (text no longer uses shell backticks).");
            }
        }
        sendReviewTextToTerminal(terminal, copiedText);
        vscode.window.showInformationMessage(usedClaudeCommand
...
async function openClaudeTerminalViaExtension() {
    ...
    let settled = false;
    let settle;
    const waitNew = new Promise((resolve) => {
        settle = (value) => {
            if (settled) {
                return;
            }
    ...
    launchSub.dispose();
    const sawClaudeLaunchOnResolvedTerminal = Boolean(terminal && claudeLaunchTerminals.includes(terminal));
    if (terminal) {
        await sleep(250);
    }
